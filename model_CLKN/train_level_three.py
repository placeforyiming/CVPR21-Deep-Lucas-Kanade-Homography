...
parser.add_argument('--dataset_name', action="store", dest= "dataset_name",default="MSCOCO",help='MSCOCO, remote_sensing,remote_sensing_no_rotation_small,remote_sensing_no_rotation')
...
parser.add_argument('--epoch_start', action="store", dest="epoch_start", type=int, default=2,help='train from which epoch')

parser.add_argument('--alpha', action="store", dest="alpha", type=int, default=1.0,help='alpha')

parser.add_argument('--epoch_num', action="store", dest="epoch_num", type=int, default=9,help='how many epochs to train')
...
if input_parameters.epoch_start>1:
    #load weights
    level_three_network.load_weights(save_path + 'epoch_'+str(input_parameters.epoch_start-1)+"_full")
...
for current_epoch in range(input_parameters.epoch_num):
    ...
    for iters in range(10000000):
        ...
        if len(np.shape(input_img))<2:
          level_three_network.save_weights(save_path +'epoch_'+str(input_parameters.epoch_start+current_epoch)+"_full")
          break
        ...
        updated_matrix_two_full_back=construct_matrix(updated_matrix_two,scale_factor=2,batch_size=input_parameters.batch_size)

        with tf.GradientTape() as tape:
            input_feature=level_three_network.call(input_img)
            template_feature=level_three_network.call(template_img)

            try:
              updated_matrix_three=LK_layer_three.update_matrix(template_feature,input_feature,updated_matrix_two)
            except:
              print ('qu ni ma de')
            ...
            total_loss=loss_function(input_parameters.batch_size,updated_matrix_two_full_back,updated_matrix_three,u_list,v_list, input_parameters.alpha,top_left_u=0,top_left_v=0,bottom_right_u=127,bottom_right_v=127)

          
        grads = tape.gradient(total_loss, level_three_network.trainable_variables)
        if input_parameters.dataset_name=='remote_sensing' or input_parameters.dataset_name=='remote_sensing_no_rotation':
            
            grads=[tf.clip_by_value(i,-1.0,1.0) for i in grads]
        optimizer.apply_gradients(zip(grads, level_three_network.trainable_variables))

        error_ave_1000=error_ave_1000+total_loss
        
        cornner_error+=np.sqrt(average_cornner_error(input_parameters.batch_size,updated_matrix_three,u_list,v_list,top_left_u=0,top_left_v=0,bottom_right_u=127,bottom_right_v=127)/4.0)
        ...
        if iters%input_parameters.save_eval_f==0 and iters>0:
            ...
            level_three_network.save_weights(save_path +'epoch_'+str(input_parameters.epoch_start+current_epoch)+"_"+str(iters))

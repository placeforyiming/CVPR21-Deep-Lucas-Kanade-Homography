...
parser.add_argument('--dataset_name', action="store", dest= "dataset_name",default="GoogleMap",help='MSCOCO, remote_sensing_no_rotation_small,remote_sensing,remote_sensing_no_rotation')
...
parser.add_argument('--epoch_load', action="store", dest="epoch_load", type=int, default=20,help='load the epoch number from level one')


parser.add_argument('--epoch_start', action="store", dest="epoch_start", type=int, default=1,help='train from which epoch')

parser.add_argument('--alpha', action="store", dest="alpha", type=int, default=4,help='alpha')

parser.add_argument('--epoch_num', action="store", dest="epoch_num", type=int, default=20,help='how many epochs to train')
...
level_one_network.load_weights(load_path + 'epoch_'+str(input_parameters.epoch_load)+"_full")

level_two_network=Level_two()
if input_parameters.epoch_start>1:
    #load weights
    level_two_network.load_weights(save_path + 'epoch_'+str(input_parameters.epoch_start-1)+"_full")
...
for current_epoch in range(input_parameters.epoch_num):
    ...
    for iters in range(10000000):
        ...
        if len(np.shape(input_img))<2:
          level_two_network.save_weights(save_path +'epoch_'+str(input_parameters.epoch_start+current_epoch)+"_full")
          break
        ...
        updated_matrix_one_full_back=construct_matrix(updated_matrix_one,scale_factor=4,batch_size=input_parameters.batch_size)

        with tf.GradientTape() as tape:
            input_feature=level_two_network.call(input_img)
            template_feature=level_two_network.call(template_img)



            updated_matrix_two=LK_layer_two.update_matrix(template_feature,input_feature,updated_matrix_one)
            #print (updated_matrix)

            updated_matrix_two=construct_matrix(updated_matrix_two,scale_factor=4,batch_size=input_parameters.batch_size)

            
            total_loss=loss_function(input_parameters.batch_size,updated_matrix_one_full_back,updated_matrix_two,u_list,v_list, input_parameters.alpha,top_left_u=0,top_left_v=0,bottom_right_u=127,bottom_right_v=127)

          
        grads = tape.gradient(total_loss, level_two_network.trainable_variables)
        optimizer.apply_gradients(zip(grads, level_two_network.trainable_variables))

        error_ave_1000=error_ave_1000+total_loss
       
        cornner_error+=np.sqrt(average_cornner_error(input_parameters.batch_size,updated_matrix_two,u_list,v_list,top_left_u=0,top_left_v=0,bottom_right_u=127,bottom_right_v=127)/4.0)
        ...
        if iters%input_parameters.save_eval_f==0 and iters>0:
            ...
            level_two_network.save_weights(save_path +'epoch_'+str(input_parameters.epoch_start+current_epoch)+"_"+str(iters))

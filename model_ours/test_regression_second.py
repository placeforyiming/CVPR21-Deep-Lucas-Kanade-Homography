...
parser.add_argument('--epoch_load_one', action="store", dest="epoch_load_one", type=int, default=50,help='epoch_load_one which epoch to load')

parser.add_argument('--epoch_eval', action="store", dest="epoch_eval", type=int, default=50,help='train from which epoch')
...
regression_network_two.load_weights(save_path + 'epoch_'+str(input_parameters.epoch_eval))
...
for iters in range(10000000):
    ...
    gt_vector,four_cornners=gt_motion_rs(homography_vector_one,u_list,v_list,batch_size=1)

    matrix=construct_matrix(1,homography_vector_one)
    ...
    with tf.GradientTape() as tape:
        ...
        loss_1=loss_func(1,four_cornners,homography_vector_two,u_list,v_list)
